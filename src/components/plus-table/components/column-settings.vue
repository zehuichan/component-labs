...
import type { SettingItem } from '../composables/use-columns';

defineOptions({ name: 'PlusTableColumnSettings' });

const table = usePlusTable();

const items = computed(() => table.settingItems.value);
...
function handleToggle(id: string, checked: boolean | string | number) {
  ...
  table.toggleColumnVisible(id, !!checked);
}
...
function columnWidth(item: SettingItem): number | undefined {
  ...
  const raw = item.id in widths ? widths[item.id] : table.getColumnById(item.id)?.column.width;
...
function handleWidthChange(item: SettingItem, width: number | null | undefined) {
  ...
  table.setColumnWidth(item.id, next ? null : width);
}
...
function move(item: SettingItem, direction: -1 | 1) {
  ...
  table.updateColumnOrder(item.id, target.id, direction < 0 ? 'before' : 'after');
}
...
function handleDrop(event: DragEvent, item: SettingItem) {
  if (!canDropOn(item)) return;
  event.preventDefault();
  table.updateColumnOrder(dragItem.value!.id, item.id, dropPosition.value);
  handleDragEnd();
}
  ...
  >
    ...
    <template #footer>
      <div class="ptbl-column-settings__footer">
        <el-button @click="table.resetSettings()"> 恢复默认 </el-button>
