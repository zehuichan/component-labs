...
import type { CellRule, ColumnNode, DependencyApi, PlusTableColumn, RowData } from '../types';
...
type UseDependenciesProps<T extends RowData> = Pick<UseDataReturn<T>, 'keysMap' | 'getRowKey'> &
  Pick<UseColumnsReturn<T>, 'triggerIndex'> & {
    /** 单元格写值流水线（由 useTable 以函数声明提供，只在运行期被调用） */
    setCellValue: (row: T, rowIndex: number, prop: string, value: unknown) => void;
  };

/** 字段联动：按 (rowKey, colId) 缓存联动状态的 computed，并在字段提交后广播 trigger 副作用。 */
export function useDependencies<T extends RowData = RowData>({
  keysMap,
  getRowKey,
  triggerIndex,
  setCellValue,
}: UseDependenciesProps<T>) {
  /**
  ...
  function makeApi(row: T, rowIndex: number, prop: string): DependencyApi<T> {
    return {
      ...
      setValue: (targetProp, value) => {
        setCellValue(row, rowIndex, targetProp, value);
      },
    };
  }
  ...
  function getDependencyState(row: T, rowIndex: number, node: ColumnNode<T>): DependencyState {
    ...
    const resident = keysMap.value.get(rowKey);
  ...
  /** 字段提交后只广播给反向索引里声明了该字段的列（含被列设置隐藏的列），执行 trigger 副作用 */
  function notifyFieldChange(row: T, rowIndex: number, changedProp: string) {
    const dependents = triggerIndex.value.get(changedProp);
...
export type UseDependenciesReturn<T extends RowData = RowData> = ReturnType<
  typeof useDependencies<T>
>;
